<?php
  ...
  $bcs = breadCrumbs($idTin);
  $row_bcs = mysqli_fetch_assoc($bcs);
  // echo "<pre>";
  // var_dump($row_tin);
  // echo"</pre>";
  ?>
<!-- Trang Chủ >>  -->
<div>Trang Chủ >> <?php echo $row_bcs['TenTL'] ?> >> <?php echo $row_bcs['Ten'] ?></div>
...
<!-- phần nút like ở đây -->
<form method="post">
<input type="hidden" name="hidden_idTL" value="<?php echo $row_tin['idTL'] ?>">
<input type="hidden" name="hidden_idLT" value="<?php echo $row_tin['idLT'] ?>">
<button type="submit" class="btn_quantam" id="vne-like-anchor-1000000-3023795" name="like" total-like="21"></button>
</form>
